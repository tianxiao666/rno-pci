...
$(document).ready(
	...
	$("#addTask").click(function() {
		...
		// $("#azimuth_calcWinDiv #_latestAllowedTime").val(new Date().format('yyyy-MM-dd HH:mm:ss'));
		$("#azimuth_calcWinDiv #_latestAllowedTime").val(new Date().format('yyyy-MM-dd'));
		
		var input = $(this);            
        var offset = input.offset();
        //先后设置div的内容、位置，最后显示出来（渐进效果）
        //$('#'+divId).css('left',offset.left + input.width() + 2 + 'px').css('top',offset.top + 'px');
        $("#azimuth_calcWinDiv").css('left',offset.left + input.width() + 2 + 'px').css('top',offset.top + 'px');
...
function displayReportRec(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html+="<td>"+getValidValue(one['begTime'],'')+"</td>";
